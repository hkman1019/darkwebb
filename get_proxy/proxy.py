...
def prox():
  print('''
[1] Get the proxy from the site (free-proxy-list.com)
[2] Download proxy from site (free-proxy-list.net) [\ 033 [92m Recommended \ 033 [0m]
[3] From the file
  ''')
  ask = int(
    input(
...
def proxy_com():
  ...
  if len(
    ...
    if len(
      ...
      '[!] Sorry there is no valid proxy, please try again :('
    )
  else: exit(
    '[!] Sorry the proxy does not exist :('
  )
...
def from_file():
  print(
    '\n[!] Seperator ip:port ex: 10.1.3:8080'
  )
  list = input(
